...
@Component({
  ...
  styleUrls: ['./invoices-sent.component.scss'],
})
export class InvoicesSentComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.invoicesService.sentInvoices$.subscribe(
      (invoices) => {
  ...
  /**
   * Load sent invoices from backend
   */
  loadSentInvoices(): void {
    this.invoicesService.getSentInvoices().subscribe();
  }
